...
bool QuickHull3D::Build(const Array<Vec3>& points, void* stack)
{
...
void QuickHull3D::Clear()
{
  ...
  for (size_t i = 0; i < mEdgePool.Size(); ++i)
    delete mEdgePool[i];
  for (size_t i = 0; i < mFacePool.Size(); ++i)
    delete mFacePool[i];
  mVertexPool.Clear();
  mEdgePool.Clear();
  mFacePool.Clear();
...
{
  QuickHullVertex* result = new QuickHullVertex();
  mVertexPool.PushBack(result);
  return result;
}

QuickHull3D::QuickHullEdge* QuickHull3D::AllocateEdge()
{
  QuickHullEdge* result = new QuickHullEdge();
  mEdgePool.PushBack(result);
  return result;
}

QuickHull3D::QuickHullFace* QuickHull3D::AllocateFace()
{
  QuickHullFace* result = new QuickHullFace();
  mFacePool.PushBack(result);
  return result;
}
